...
void ExoticBSEngine::GenOnePath(MJArray& SpotValues) {
    ...
    for (unsigned long j=0; j<NumberOfTimes; j++) {
        ...
        CurrentLogSpot += StandardDerivations[j] * Variates[j];
    ...
    TheGenerator(TheGenerator_)
{
    ...
    for (unsigned long j=1; j<NumberOfTimes; j++) {
        ...
        StandardDerivations[j] = sqrt(thisVariance);
    }
    LogSpot = log(Spot_);
